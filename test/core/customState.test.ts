...
describe('自定义状态扩展', () => {
  describe('pausedState - 暂停状态', () => {
    it('用户应该能够扩展暂停状态', async () => {
      ...
      await manager.pauseObject(object.id as string)

      const retrieved = await manager.get(object.id as string)
      expect(retrieved?.state.name).toBe('paused')
    })
  ...
  describe('errorState - 错误状态', () => {
    it('用户应该能够扩展错误状态', async () => {
      ...
      const object = await manager.create()
      await manager.errorObject(object.id as string, 'Something went wrong')

      const retrieved = await manager.get(object.id as string)
      expect(retrieved?.state.name).toBe('error')
      expect((retrieved?.state as ErrorState).errorMessage).toBe('Something went wrong')
    })
    ...
    it('错误状态应该能携带额外信息', async () => {
      ...
      const object = await manager.create()
      await manager.errorObject(object.id as string, 'Network timeout')

      const retrieved = await manager.get(object.id as string)
      const errorState = retrieved?.state as ErrorState
  ...
  describe('completedState - 完成状态', () => {
    it('用户应该能够扩展完成状态', async () => {
      ...
      const object = await manager.create()
      await manager.completeObject(object.id as string)

      const retrieved = await manager.get(object.id as string)
      expect(retrieved?.state.name).toBe('completed')
    })
  })
  ...
  describe('直接使用 LifecycleState 接口', () => {
    it('用户可以直接创建并使用自定义状态', async () => {
      ...
      object.state = customState

      expect(object.state.name).toBe('paused')
    })
